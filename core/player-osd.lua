-- player.lua: HUD system using OSD overlay instead of subtitles
...
-- Create HUD overlay
local function create_hud_overlay()
    if hud_overlay then
        hud_overlay:remove()
    end
    ...
    -- Create overlay
    hud_overlay = mp.create_osd_overlay("ass-events")

    -- Determine colors based on effects
    local color = "00FFFF" -- Cyan for cyberpunk
    for _, effect in ipairs(config.effects or {}) do
        if effect:match("vhs") then
            color = "FFAA00" -- Orange for VHS
            break
        end
    end

    -- Position the HUD
    local y_pos = (position == "top") and 50 or 950
    local alignment = (position == "top") and 8 or 2

    hud_overlay.data = string.format(
        "{\\an%d\\pos(960,%d)\\fs24\\c&H%s&\\bord2\\shad1}%s",
        alignment, y_pos, color, hud_text
    )
...
local function show_message()
    ...
    -- Use simple OSD message for now
